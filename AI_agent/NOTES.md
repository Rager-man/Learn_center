...
- **第 2 课 ✓ 结业（2026-09-03）**：结构化输出（智谱主线）。两任务完成：ex3 招式一二合体 + 三级兜底（parseReview / BadJSON / 原样重试 / 回传修复）。正式压测 9/9 一次成功零裸崩（兜底未触发——json_object 之下失败罕见）；复盘题 2 过、**复盘题 1 挂起**（学员未亲历失败，导师代跑诱导补测采得 5 种真实失败、留痕于 ex3 尾注，待学员对号口述补判）。判卷发现与伏笔见下方课记录。
...
- 概念方向感本次无回潮（复盘题 2 的输出方向表述正确）。新观察：**"太稳"也是一种坑**——学员因从未见过失败而对兜底无体感，复盘题 1 的挂起处理即为此设计。第 3 课开场先回收两件事：① 复盘题 1 对号口述；② 诱导补测 5 种失败各自该走兜底阶梯哪一级。
...
**待办**：复盘题 1 对号口述补判（学员）→ 第 3 课课件制作（`02_ToolUse/ex4_questions.py` TS 化 + 骨架预建）→ 开课。
