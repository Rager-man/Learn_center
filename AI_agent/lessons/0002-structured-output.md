...
> 第 1 课你亲手证明了"没发的，等于不存在"——还顺手发明了 `/delete`。今天看同一枚硬币的另一面：**发回来的，也不能全信**。
...
第 1 课的 ex1/ex2 你已经跑通——地基有了，直接谈结构化。

**开场复查（10 秒）**：两条上一课的现场记录——① 复盘判卷时你在 messages 的方向上差过半步：它是你**发给** API 的请求体，不是 API 返回给你的。今天 §1 的 `content` 就是这个请求体换来的响应正文，方向别再反。② **key 只进环境变量，别写进代码**——上一课的教训还热乎。

三个环境变量（新开终端要重设，想一劳永逸就写进 `~/.zshrc`）：
...
今天应该是绿的（ex3 的地基骨架类型干净）；任务 2 写 `BadJSON` 等新代码时若报红，修到沉默为止。

本课练习文件（`01_API基础/`）：

- `ex3_reviews.ts` —— 任务 1：评论分类器（60 分钟）+ 任务 2：加兜底（20 分钟）。地基骨架在 §3，文件你来建；任务 2 的待写区（TODO + 完成判据）已标在文件尾部
...
1. **content 是字符串，不是对象。**长得像 JSON 的文本依然是文本——程序要用它，必须先过 `JSON.parse` 这道门。
2. **这道门随时会锁上。**失败不是小概率异常，是常规路径：线上跑一万条，每一种失败方式都会撞上——兜底是常态代码，不是防御性姿态。
3. **类型管不到运行时。**你给函数标 `Promise<Review>` 是写给编译器的单方面合同，`JSON.parse` 出来的东西长什么样它不查——编译期、运行期两道墙，都要有。
...
> ✅ **检查点（比写完代码更重要）**
> 手里有一份**真实的**失败清单（第 3 步记录、第 4 步更新）——复盘题 1 的答案从这里来，别编，要真见过。顺带数一数"解析成功但结果离谱"的条目：解析成功，不代表结果可用。
...
- [x] 两招都实测过，各有一份失败清单
- [x] 在响应里亲手认出 `reasoning_content`（思维链）和 `content`（正文），说得出谁计费、取哪个
- [x] `parseReview` 能剥围栏、截花括号
- [x] 回传修复的 messages 拼对了——能指出坏输出是哪条 role、报错是哪条 role
- [x] 能一句话说清：为什么标了 `Review` 类型，运行时校验照样不能省
- [x] 全部测试评论跑完，程序零裸崩
