...
- [ ] ex12：三条错误路径的打印分得清是哪一条拦下的
- [ ] ex12：三连实测留痕齐全（坏主机名 / 纬度 999 / schema 改错），实测后代码已改回
- [ ] ex12：`res.json()` 结果注解了 `unknown`；类型全部来自 `z.infer`
- [ ] ex12：`safeParse` 失败信息带 `message` 和 `path`
- [ ] ex13：进口清单写全，每个进口有校验标注
- [ ] ex13：`parseCoords` 三关生效；坏输入补前/补后对比留痕 + 一句结论
