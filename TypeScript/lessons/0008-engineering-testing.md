...
- [ ] ex16：两个模块迁移完成——只多 export、逐字一致；ex7 / ex14 未动
- [ ] ex16：≥8 个 it 全绿，必测清单全覆盖（非法流转×2、终态、空数组、可选属性、`n <= 0` 边界……）
- [ ] ex16：toBe / toEqual 各至少一次且用对；破坏实验留痕四行 + 现场恢复全绿
- [ ] ex17：三间屋拆分到位，import 全带 `.js`、零死 import
- [ ] ex17：`npm run dev -- 39.9 116.4` 跑通；错误路径复测 + 依赖方向留痕；ex12 未动
...
*上一课：[第 7 课 · 泛型与工具类型](0007-generics-utility-types.md) ｜ 下一课：第 9 课 · 综合实战（完成本课后解锁）*
