...
let round_count = 0; // 轮数计数器，乱敲的不算一轮
// TODO 2) 游戏循环，反复读输入：
//   - 输入 "q"（大小写都认）→ 打印一句认输的话，退出循环
//   - Number(...) 解析出 NaN → 打印"这不是一个数字"，continue 重新读（第 1 课的坑在这里补完）
//   - 是数字 → 进 TODO 3
while (1) {
  const line = (await rl.question("猜一个 1–100 的数（q 退出）：")).trim();
  const guess_number = Number(line);

  if(line.toLowerCase() === 'q') {
    console.log(`认输啦！正确答案是 ${answer_number}`);
    break;
  }else if(Number.isNaN(guess_number)) {
    console.log("这不是一个数字");
    continue;
  }
...
//   - 每读一次有效输入算一轮（乱敲的不算——想想轮数计数器该加在哪一行）
    ...
    if(guess_number > answer_number) {
      ...
      console.log(`猜中了！一共用了 ${round_count} 轮`);
      break;
    }
  }
}
